...
from db.schemas import LFBase, LFDisplay, Condition, ConditionShow
...
def add_staff (db: Session, request: LFBase, farm_id: int):
    ...
    return new_staff
def add_condition (db: Session, request: Condition, farm_id: int):
...
def get_farm_with_condition (db: Session):
    results = db.query(
        ...
        dbFarm.name,
        dbFarm.owner,
        dbFarm.register_date.label("register_date"),
