...
from router import register_user, login, delete_user, statistic, search_accounts, add, create_group
...
#Route rút gọn cho trang home của guest
@app.get("/home_guest", response_class=HTMLResponse)
async def read_home_guest():
    file_path = Path("static/html/guest.html")
    if file_path.is_file():
        return file_path.read_text(encoding='utf-8')
    return HTMLResponse("<h1>File not found</h1>", status_code=404)
...
#Rút gọn cho trang guest_user
@app.get("/guest_user", response_class=HTMLResponse)
async def read_guest_user():
    file_path = Path("static/html/guest_user.html")
    if file_path.is_file():
        return file_path.read_text(encoding='utf-8')
    return HTMLResponse("<h1>File not found</h1>", status_code=404)
